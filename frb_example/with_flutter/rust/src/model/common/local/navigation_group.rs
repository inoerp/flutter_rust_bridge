...
impl NavigationGroup {
        ...
        pub async fn find_by_sql(
            ...
            sql: &str,
        ) -> Result<Vec<NavigationGroup>, Box<dyn Error>> {
            if let Some(pool) = sqlite_pool {
                let mut connection = pool.acquire().await?;
                let data_rows: Vec<NavigationGroup> = sqlx::query_as::<_, NavigationGroup>(sql)
        ...
        pub async fn find_all(
            ...
            let sql: &str = "SELECT * from navigation_group ";
        ...
        pub async fn find_by_id(
            ...
            let sql1: String = format!("SELECT * from navigation_group WHERE id = '{}' ", id);
    ...
    sqlite_pool: Option<&Pool<sqlx::Sqlite>>,
        ...
        let mut sql: String = "SELECT * from navigation_group WHERE  1 = 1 ".to_string();
        ...
        let mut x1 = sqlx::query_as::<_, NavigationGroup>(&sql);
        ...
        if let Some(pool) = sqlite_pool {
        let mut connection = pool.acquire().await?;
            let data_rows: Vec<NavigationGroup> = x1.fetch_all(&mut connection).await?;
            Ok(data_rows)
        } else {
...
async fn test_find_all() {
    let data = NavigationGroup::find_all(Option::None).await;
    match data {
        Ok(d) => {
...
async fn test_find_by_id() {
    let data = NavigationGroup::find_by_id(Option::None,"1").await;
    match data {
        Ok(d) => {
...
async fn test_find_by_params() {
    ...
    let data = NavigationGroup::find_by_params(Option::None, params).await;
    match data {
        Ok(d) => {
            log::info!("found data. length {:?}", d);
        }
