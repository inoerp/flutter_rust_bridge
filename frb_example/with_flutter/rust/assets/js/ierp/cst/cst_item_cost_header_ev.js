...
function changePeriodStatus(data) {
  ...
  let sql = " select cstStandardCostUpdate(" + cstItemCostHeaderId + ")";
...
function getChangedCostAmount(data) {
  ...
  let costChangeSql =
    ...
    " from cst_item_cost_header_ev ich " +
    " LEFT JOIN cst_current_frozen_cost_v cfc ON cfc.inv_org_id = ich.inv_org_id " +
...
function createInventoryTransaction(data, changedCost) {
  ...
  //Find the OH Quantity and WIP Quantity
  let onHandSql =
    " SELECT SUM(onhand) as onhand FROM inv_onhand_summary_v " +
    " where inv_org_id = '" +
    data["invOrgId"] +
