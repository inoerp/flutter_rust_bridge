...
function createHrPayslipLines(hrPayrollProcessId, compHeaderIds) {
  const selectSql =
    " SELECT * FROM hr_payslip_header where doc_status = 'entered' and hr_payroll_process_id = '" +
    hrPayrollProcessId +
    "' ";
  const sqlRequest = {
    sql: selectSql,
...
function getAllCompensationLines(compHeaderIds) {
  const selectSql =
    "SELECT * FROM hr_compensation_line where hr_compensation_header_id in (" +
    compHeaderIds +
    ")";
  const sqlRequest = {
    sql: selectSql,
...
function getHrPayrollProcessData(hrPayrollProcessId) {
  const selectSql =
    " SELECT * from hr_payroll_process_v where hr_payroll_process_id = '" +
    hrPayrollProcessId +
    "'  ";
  const sqlRequest = {
    sql: selectSql,
...
function getEmployeeData(hrPayrollProcessId) {
  const employeeSelectSql =
    " SELECT * FROM hr_employee_ev where vv_emp_hr_payroll_id IN \
