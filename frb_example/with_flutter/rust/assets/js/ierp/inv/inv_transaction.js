...
function getJournalProfileAndCreateAccounting(flatObject, invTransactionId) {
  ...
  let sql =
    " SELECT jpl.* FROM " +
    " gl_journal_profile_l jpl, " +
    " gl_journal_profile_h jph, " +
    " gl_journal_pro_assg jpa, " +
  ...
  let headerSql =
    ...
    invTransactionId +
    "'" +
    " FROM inv_transaction_ev WHERE inv_transaction_id = '" +
    invTransactionId +
    "'; ";
...
function getAccountProfileLines(flatObject) {
  ...
  if (
    ...
    sql =
      "SELECT * FROM gl_ac_profile_line where gl_ac_profile_header_id = '" +
      glAcProfileHeaderId +
      "' ;";
  } else {
