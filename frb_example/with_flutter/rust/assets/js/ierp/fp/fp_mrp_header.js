...
function createMRP(fpMrpHeaderId, lowLevelCode) {
  ...
  let supplyRowsMap = {};
  let demandSql =
    " SELECT * FROM fp_mrp_demand_v     WHERE demand_date < now() + INTERVAL 365 DAY " +
    " AND fp_mrp_header_id = '" +
    fpMrpHeaderId +
    "' " +
  ...
  let supplySql =
    " SELECT * FROM fp_mrp_supply     WHERE supply_date < now() + INTERVAL 365 DAY " +
    " AND fp_mrp_header_id = '" +
    fpMrpHeaderId +
    "' " +
    " AND inv_item_master_id in (SELECT inv_item_master_id from fp_mrp_lowlevel_code where low_level = '" +
...
function createDependantDemands(plannedOrderList) {
  ...
  let selectSql =
    " SELECT * FROM bom_line_ev WHERE vv_inv_org_id = '" +
    invOrgId +
