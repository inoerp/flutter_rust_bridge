...
function createAssignment(action, inputAssetId) {
  ...
  if (action == "fa_depreciation") {
    ...
    selectSql +=
      " AND vv_fa_asset_category_id IN (SELECT fa_asset_category_id from fa_asset \
       WHERE fa_asset_id = '" +
      inputAssetId +
      "' ) ";
  } else {
    selectSql +=
      " AND vv_legal_org_id IN (SELECT legal_org_id from fa_asset \
       WHERE fa_asset_id = '" +
      inputAssetId +
      "' ) ";
  }
...
function getAssetDetails(faAssetId) {
  let selectSql =
    "SELECT * FROM fa_asset WHERE fa_asset_id = '" + faAssetId + "' ";
...
function getDepreciationRates() {
  let selectSql = "SELECT * FROM fa_depreciation_method_rate_ev ";
...
function createStatusTransaction(transactionType, faAssetId, action) {
  ...
  if (action == "fa_status_active") {
    ...
    insertSql +=
      " AND vv_asset_status = 'pending_addition' AND vv_fa_asset_category_id IN (SELECT fa_asset_category_id from fa_asset \
       WHERE fa_asset_id = '" +
      faAssetId +
      "' ) ";
  } else if (action == "fa_status_active_legal_org") {
    insertSql +=
      " AND vv_asset_status = 'pending_addition' AND vv_legal_org_id IN (SELECT legal_org_id from fa_asset \
     WHERE fa_asset_id = '" +
      faAssetId +
      "' ) ";
  } else if (action == "fa_status_retire") {
...
function getFaJournalProfiles() {
  let selectSql =
    " SELECT fat.*, gjpl.* \
  FROM \
  fa_transaction_type fat \
  join gl_journal_profile_h gjph ON gjph.profile_code = fat.gl_document_type \
  join  gl_journal_profile_l gjpl ON gjpl.gl_journal_profile_h_id =  gjph.gl_journal_profile_h_id ";
