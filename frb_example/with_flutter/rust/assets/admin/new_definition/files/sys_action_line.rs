...
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, PartialOrd)]
        pub struct SysActionLine {pub sys_action_line_id : i32,
...
fn get_entity(
        ...
        ) -> Result<SysActionLine , NoValueFoundError> { let sys_action_line_id: i32 = row
                    .get("sysActionLineId")
                    .ok_or_else(|| NoValueFoundError::new("Missing sysActionLineId"))?
                    .parse()
                    .map_err(|_| NoValueFoundError::new("Invalid sysActionLineId"))?; let src_entity_name: Option<String> = row.get("srcEntityName").map(|v| v.to_string());let src_entity_id: Option<String> = row.get("srcEntityId").map(|v| v.to_string());let sys_action_id: i32 = row
                    .get("sysActionId")
                    .ok_or_else(|| NoValueFoundError::new("Missing sysActionId"))?
                    .parse()
                    .map_err(|_| NoValueFoundError::new("Invalid sysActionId"))?; let sequence: i32 = row
                    .get("sequence")
                    .ok_or_else(|| NoValueFoundError::new("Missing sequence"))?
                    .parse()
                    .map_err(|_| NoValueFoundError::new("Invalid sequence"))?; let src_table_name: Option<String> = row.get("srcTableName").map(|v| v.to_string());let src_table_id: Option<String> = row.get("srcTableId").map(|v| v.to_string());let dst_table_name: Option<String> = row.get("dstTableName").map(|v| v.to_string());let dst_table_id: Option<String> = row.get("dstTableId").map(|v| v.to_string());let line_action_code: Option<String> = row.get("lineActionCode").map(|v| v.to_string());
...
let sys_action_line_id_onhold= match sys_id_1 {
                        ...
                        None => None,
                    }; let activity_path: Option<String> = row.get("activityPath").map(|v| v.to_string());let description: Option<String> = row.get("description").map(|v| v.to_string());
...
let is_go_routine= match sys_id_1 {
                        ...
                        None => None,
                    }; let created_by: String = row.get("createdBy").ok_or_else(|| NoValueFoundError::new("Missing createdBy"))?
...
let last_update_date: NaiveDateTime =
 ...
 Ok(SysActionLine { sys_action_line_id,src_entity_name,src_entity_id,sys_action_id,sequence,src_table_name,src_table_id,dst_table_name,dst_table_id,line_action_code,sys_action_line_id_pass,sys_action_line_id_fail,sys_action_line_id_onhold,activity_path,description,is_go_routine,created_by,creation_date,last_updated_by,last_update_date, }) 
}
...
mod tests {
    use crate::{
        ...
        model::{entity::action::sys_action_line::SysActionLine, state::global_state::GlobalState}, startup,
    };
    ...
    async fn test_find_all() {
        ...
        let all_data = SysActionLine::find_all(conn).await.expect("Unable to find result");
