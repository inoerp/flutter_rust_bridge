function createSchedulesForMeters(
  ...
  let headerSelectSql = " SELECT * FROM am_meter_header WHERE 1 = 1 ";
  ...
  let lineSelectSql = " SELECT * FROM am_meter_line WHERE 1 = 1 ";
  if (allMeterIds.length > 0) {
    lineSelectSql +=
      " AND am_meter_header_id IN (" + allMeterIds.join(",") + ")";
  }
  ...
  let maximumReadingSql =
    " SELECT * FROM am_meter_reading_max_value_v WHERE 1 = 1 ";
  if (allMeterIds.length > 0) {
    maximumReadingSql +=
      " AND am_meter_header_id IN (" + allMeterIds.join(",") + ")";
  }
