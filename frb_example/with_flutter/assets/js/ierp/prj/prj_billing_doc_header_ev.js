...
function getAllJournalProfiles() {
  let selectSql =
    " SELECT gjpl.*       \
      FROM        gl_journal_profile_l gjpl \
      join gl_journal_profile_h gjph ON gjpl.gl_journal_profile_h_id =  gjph.gl_journal_profile_h_id \
...
function generateDraftRevenueForPOC(
  ...
  FROM prj_project_cost_v WHERE prj_project_header_id = '" +
    prjProjectHeaderId +
    "' ";
