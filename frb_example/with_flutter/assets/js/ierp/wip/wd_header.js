function afterPost(inputData) {
  ...
  if (wipHeaderId) {
    let insertSql =
      ...
      "  SELECT bom_routing_header_id from wip_wd_header where wip_wd_header_id = '" +
      wipHeaderId +
      "'" +
      "  ) ";
...
function insertOperationResources(wipWdLineId) {
  ...
  let insertSql =
    ...
    " FROM wip_wd_line WHERE wip_wd_line_id = '" +
    wipWdLineId +
    "'); ";
  request = {
    sql: insertSql,
...
function insertOperationMaterials(wipWdLineId, invItemMasterId, invOrgId) {
  ...
  let insertSql =
    ...
    " SELECT bom_header_id FROM bom_header WHERE inv_item_master_id = '" +
    invItemMasterId +
    "' and inv_org_id = '" +
    invOrgId +
    "') ;";
